...
\begin{Arguments}
\begin{ldescription}
\item[\code{x }] numeric vector or an object on which \code{residuals} can be used such as lm, glm, aov , gam, etc.
\item[\code{title }] the title at the top of the results. Default is "Normality Tests".
\item[\code{type }] type of skewness used in D'Agostino skewness test. Can be \code{"G1"},\code{"b1"} or \code{"mc"} (default). Read details.
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
