...
\item an object of class \code{"norm"} resulting from a
call to \code{\LinkA{norm.test}{norm.test}}
\end{itemize}
...
\newpage
\inputencoding{utf8}
\HeaderA{plot.mat}{Standard plot for maturity ogive}{plot.mat}
\keyword{\textbackslash{}textasciitilde{}kwd1}{plot.mat}
\keyword{\textbackslash{}textasciitilde{}kwd2}{plot.mat}
%
\begin{Usage}
\begin{verbatim}
xxx( data ,  ,  ,  )
...
\item[\code{age.step}] steps used to generate ages (\code{0:last.age}). Default is \code{1}.
...
\item[\code{F.max}] maximum value of instantaneous rate of fishing mortality (\code{F}). Default is \code{2}.

\item[\code{F.incr.YPR}] increment for generating the \code{F} values to be used for YPR calculation. Default is \code{0.0001}.

\item[\code{M}] instantaneous rate of natural mortality (\code{M}). Default is \code{0.2}.
...
\item[\code{f.MSP}] reference point defined as the fraction of maximum spawning potential. Default is \code{0.4}.
