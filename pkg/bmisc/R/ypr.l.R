...
ypr.l <- function(LW, vonB, l.start, last.age, age.step=1, prop.surv=NULL , fish.lim=NULL ,Fsel.type=NULL, F.max=2, F.incr.YPR=0.0001, 
        Mat,  Msel.type=NULL, M=0.2, f.MSP=0.4){ #, F.f=0, M.f=0.5, riv.calc=FALSE){
    ...
    if(is.null(Mat)){ mat <- const.sel( x = YPR$l.age)
    }else{
        if(!(class(Mat)[1] %in% c("list","glm"))) stop("Fsel.type should either be a list or a glm (logit) object.")
    }
    if(class(Mat)[1]=="list"){
        switch(Mat[[1]],
                const=  mat <- const.sel( x = YPR$l.age),
                full= {
                    if(all(!("infl1" %in% names(Mat)))) stop("'infl1' must be defined. Read help('ypr').")
                    Mat1=list(x=YPR$l.age, infl1=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Mat1) %in% names(Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-Mat[name.sel]			
                    mat=do.call("full.sel", Mat1)					
                },
                ramp= {
                    if(all(!(c("infl1","infl2") %in% names(Mat)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Mat1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Mat1) %in% names(Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-Mat[name.sel]	
                    mat=do.call("ramp.sel", Mat1)
                },
                logit= {
                    if(all(!(c("infl1","infl2") %in% names(Mat)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Mat1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1 , prob=NULL, prop=0.1,beta=0.2, fast=TRUE)
                    sel=which(names(Mat1) %in% names(Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-Mat[name.sel]		
                    mat=do.call("logit.sel", Mat1)
                },
                mod.logit= { 					
                    if(all(!(c("alpha","beta") %in% names(Mat)))) stop("'alpha' and 'beta' must be defined. Read help('ypr').")
                    Mat1=list(x=YPR$l.age, alpha=NULL, beta=NULL)
                    sel=which(names(Mat1) %in% names(Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-Mat[name.sel]		
                    mat=do.call("mod.logit.sel", Mat1)
    ...
    if(class(Mat)[1]=="glm"){
        coeffs=coef(Mat)
        mat <- mod.logit.sel(alpha=coeffs[[1]], beta=coeffs[[2]], x=YPR$l.age)
    }
    ...
    if(is.null(Msel.type)){ M.sel <- const.sel( x = YPR$l.age)
    }else{
        if(!(class(Msel.type)[1] %in% c("list", "glm"))) stop("Msel.type should either be a list or a glm (logit) object.")
    }
    if(class(Msel.type)[1]=="list"){		
        switch(Msel.type[[1]],
                const=  M.sel <- const.sel( x = YPR$l.age),
                full= {
                    if(all(!("infl1" %in% names(Msel.type)))) stop("'infl1' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]		
                    M.sel=do.call("full.sel", Msel.type1)
                },
                plat.full= {
                    if(all(!(c("infl1","infl2") %in% names(Msel.type)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]		
                    M.sel=do.call("plat.full.sel", Msel.type1)					
                },
                ramp= {
                    if(all(!(c("infl1","infl2") %in% names(Msel.type)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1)
                    ...
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]	
                    M.sel=do.call("ramp.sel", Msel.type1)
                },
                plat.ramp= {
                    if(all(!(c("infl1","infl2", "infl3", "infl4") %in% names(Msel.type)))) stop("'infl1' to 'infl4' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, infl3=NULL, infl4=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]		
                    M.sel=do.call("plat.ramp.sel", Msel.type1)
                },
                logit= {
                    if(all(!(c("infl1","infl2") %in% names(Msel.type)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1 , 
                    ...
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]		
                    M.sel=do.call("logit.sel", Msel.type1)
                },
                plat.logit= {
                    if(all(!(c("infl1","infl2", "infl3", "infl4") %in% names(Msel.type)))) stop("'infl1' to 'infl4' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, infl1=NULL, infl2=NULL, infl3=NULL, infl4=NULL, neg=FALSE, lv=0, uv=1, 
                    ...
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]		
                    M.sel=do.call("plat.logit.sel", Msel.type1)
                },
                mod.logit= {
                    if(all(!(c("alpha","beta") %in% names(Msel.type)))) stop("'alpha' and 'beta' must be defined. Read help('ypr').")
                    Msel.type1=list(x=YPR$l.age, alpha=NULL, beta=NULL)
                    sel=which(names(Msel.type1) %in% names(Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-Msel.type[name.sel]	
                    M.sel=do.call("ramp.sel", Msel.type1)
    ...
    if(class(Msel.type)[1]=="glm"){
        coeffs=coef(Msel.type)
    ...
    if(class(Fsel.type)[1]=="glm"){
        ...
        F.sel <- mod.logit.sel(alpha=coeffs[[1]], beta=coeffs[[2]], x=YPR$l.age)
    }        
    ...
    if(!is.null(prop.surv)){
        ...
        prop.s=prop.surv(YPR$l.age)
        F.sel[sel]=F.sel[sel]*prop.s[sel]
    }
...
plot.parms.ypr <- function(object){
    ...
    if(is.null(object@parms$Mat)){ mat <- const.sel(x.dat)
    }else{
        if(!(class(object@parms$Mat)[1] %in% c("list","glm"))) stop("Fsel.type should either be a list or a glm (logit) object.")
    }
    if(class(object@parms$Mat)[1]=="list"){
        switch(object@parms$Mat[[1]],
                const=  mat <- const.sel(x.dat),
                full= {
                    if(all(!("infl1" %in% names(object@parms$Mat)))) stop("'infl1' must be defined. Read help('ypr').")
                    Mat1=list(x=x.dat, infl1=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Mat1) %in% names(object@parms$Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-object@parms$Mat[name.sel]		
                    mat=do.call("full.sel", Mat1)					
                },
                ramp= {
                    if(all(!(c("infl1","infl2") %in% names(object@parms$Mat)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Mat1=list(x=x.dat, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Mat1) %in% names(object@parms$Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-object@parms$Mat[name.sel]		
                    mat=do.call("ramp.sel", Mat1)
                ...
                logit= {
                    if(all(!(c("infl1","infl2") %in% names(object@parms$Mat)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Mat1=list(x=x.dat, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1 , prob=NULL, prop=0.1,beta=0.2, fast=TRUE)
                    sel=which(names(Mat1) %in% names(object@parms$Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-object@parms$Mat[name.sel]		
                    mat=do.call("logit.sel", Mat1)
                ...
                mod.logit= { 					
                    if(all(!(c("alpha","beta") %in% names(object@parms$Mat)))) stop("'alpha' and 'beta' must be defined. Read help('ypr').")
                    Mat1=list(x=x.dat, alpha=NULL, beta=NULL)
                    sel=which(names(Mat1) %in% names(object@parms$Mat))
                    name.sel=names(Mat1)[sel]
                    Mat1[name.sel] <-object@parms$Mat[name.sel]		
                    mat=do.call("mod.logit.sel", Mat1)					
    ...
    if(class(object@parms$Mat)[1]=="glm"){
        coeffs=coef(object@parms$Mat)
        mat <- mod.logit.sel(alpha=coeffs[[1]], beta=coeffs[[2]], x=x.dat)
    }
    ...
    if(is.null(object@parms$Msel.type)){ M.sel <- const.sel(x.dat)
    }else{
        if(!(class(object@parms$Msel.type)[1] %in% c("list", "glm"))) stop("object@parms$Msel.type should either be a list or a glm (logit) object.")
    }
    if(class(object@parms$Msel.type)[1]=="list"){		
        switch(object@parms$Msel.type[[1]],
                const=  M.sel <- const.sel(x.dat),
                full= {
                    if(all(!("infl1" %in% names(object@parms$Msel.type)))) stop("'infl1' must be defined. Read help('ypr').")
                    Msel.type1=list(x=x.dat, infl1=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(object@parms$Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-object@parms$Msel.type[name.sel]		
                    M.sel=do.call("full.sel", Msel.type1)
                },
                plat.full= {
                    if(all(!(c("infl1","infl2") %in% names(object@parms$Msel.type)))) stop("'infl1' and 'infl2' must be defined. Read help('ypr').")
                    Msel.type1=list(x=x.dat, infl1=NULL, infl2=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(object@parms$Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-object@parms$Msel.type[name.sel]			
                    M.sel=do.call("plat.full.sel", Msel.type1)					
                ...
                plat.ramp= {
                    if(all(!(c("infl1","infl2", "infl3", "infl4") %in% names(object@parms$Msel.type)))) stop("'infl1' to 'infl4' must be defined. Read help('ypr').")
                    Msel.type1=list(x=x.dat, infl1=NULL, infl2=NULL, infl3=NULL, infl4=NULL, neg=FALSE, lv=0, uv=1)
                    sel=which(names(Msel.type1) %in% names(object@parms$Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-object@parms$Msel.type[name.sel]			
                    M.sel=do.call("plat.ramp.sel", Msel.type1)
                ...
                mod.logit= {
                    if(all(!(c("alpha","beta") %in% names(object@parms$Msel.type)))) stop("'alpha' and 'beta' must be defined. Read help('ypr').")
                    Msel.type1=list(x=x.dat, alpha=NULL, beta=NULL)
                    sel=which(names(Msel.type1) %in% names(object@parms$Msel.type))
                    name.sel=names(Msel.type1)[sel]
                    Msel.type1[name.sel] <-object@parms$Msel.type[name.sel]			
                    M.sel=do.call("ramp.sel", Msel.type1)
    ...
    if(class(object@parms$Msel.type)[1]=="glm"){
        coeffs=coef(object@parms$Msel.type)
    ...
    if(!is.null(object@parms$prop.surv)){
        ...
        x.dat2=x.dat[sel]
        prop.s=object@parms$prop.surv(x.dat)
        F.sel2=F.sel
        F.sel2[sel]=F.sel[sel]*prop.s[sel]
    ...
    plot(F.sel~x.dat  , xlim=xlim, ylim=ylim, main="Fishing Slectivity\n(Fsel.type)", type='l', lwd=2.7, xlab="Length", ylab="Probability")
    if(!is.null(object@parms$prop.surv)){
