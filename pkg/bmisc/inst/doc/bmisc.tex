...
\makeindex{}
\usepackage{/Library/Frameworks/R.framework/Versions/2.13/Resources/share/texmf/tex/latex/Sweave}
\begin{document}
\chapter*{}
\begin{center}
...
\item[Depends]\AsIs{car, lattice, zoo, robustbase, methods, tcltk}
\item[License]\AsIs{LGPL >= 3.0}
\item[Repository]\AsIs{R-Forge}
\item[Repository/R-Forge/Project]\AsIs{bmisc}
\item[Repository/R-Forge/Revision]\AsIs{95}
\end{description}
...
\begin{Arguments}
\begin{ldescription}
\item[\code{obj}] the name of an object (\code{vector, matrix, data.frame, array or list})
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\item[\code{data}] \code{\LinkA{data.frame}{data.frame}} containing the dependant variable and the factor(s).
...
%
\begin{SeeAlso}\relax
\code{\LinkA{fligner.test}{fligner.test}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{Details}\relax
\bold{rprofile = \code{FALSE}}\\{}
A \code{SAVE} dialog box opens.


\bold{rprofile = \code{TRUE}}\\{}
...
\newpage
\inputencoding{utf8}
\HeaderA{plot.lev}{Plot Diagnostics for an \code{lev} Object}{plot.lev}
%
\begin{Description}\relax
...
\begin{Arguments}
\begin{ldescription}
\item[\code{obj}] an object of class \code{'lev'} resulting from a calls to \code{\LinkA{lev}{lev}}.
...
\item[\code{...}] arguments to be passed to \code{\LinkA{boxplot}{boxplot}}.
...
plot.logit(object, se.pred=TRUE, leg=TRUE, ref=TRUE, range.x=c("data","full"), 
         warn.val=TRUE, ylab="Probabilit<c3><a9>",xlab="Longueur <c3><a0> la fourche (mm)",
...
\item[\code{se.pred}] logical; if \code{TRUE}, SE is plotted.

\item[\code{leg}] logical; if \code{TRUE}, a legend containing logistic equation and estimated values for the variables is plotted.

\item[\code{ref}] logical; if \code{TRUE}, reference lines for \code{L90, L50 and L10} are plotted.

\item[\code{range.x}] the range used to define \code{xlim} in the plot. Read 'details'.

\item[\code{warn.val}] logical; if \code{TRUE} plots \eqn{x01}{} and \eqn{x99}{} when the fit is suspicious.

\item[\code{main}] an overall title for the plot. If \code{NULL} ...

\item[\code{ylab}] a title for the y axis.

\item[\code{xlab}] a title for the x axis.

\item[\code{enc.utf8}] logical; if \code{TRUE}, \code{\LinkA{iconv}{iconv}(x,"utf-8")} is used on text in the graph. Read 'details'. 

\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
When using RConsole inside of Eclipse, encoding is wrong. The use of \code{enc.utf8=TRUE} is a temporary fix for correctly 
...
\eqn{ La=L\infty ( 1-e^(-Ka))+ Ls e^(-Ka) }{}

where \eqn{La}{} is length at a relative age \eqn{a}{} and \eqn{Ls}{} is length at relative age zero.\\{}\\{}
...
%
\begin{Value}
\code{ypr} returns an object of \code{class(S4)} \code{"ypr"}. The functions \code{summary} and \code{plot} are used to respectively
...
%
\begin{SeeAlso}\relax
\code{\LinkA{plot.ypr}{plot.ypr}} and \code{\LinkA{plot.parms.ypr}{plot.parms.ypr}}  
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
