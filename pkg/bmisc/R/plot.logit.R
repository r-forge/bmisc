...
## Date: 2011-08-02															                               ##
## TODO AJOUTER UNE PARTIE QUI PERMETE DE PRENDRE LES RÉSULTATS DE find.beta() ##
##-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-##
#################################################################################
        ...
        leg=TRUE, ref=TRUE, range.x=c("data","full"), warn.val=TRUE, main,...)
{
        ...
        if(missing(main)){
                ...
                ylab=ylab, xlab=xlab , ylim=c(0,1), xlim=range(new.dat),...)
                        ...
                        title("(Â± confidence interval)", cex.main=1, line=1)
        ...
        if(ref){
                ...
                lines(c(0,L10$est,L10$est), c(L10$y,L10$y,-1), lty=2)
                        ...
                        text(x=L50$est, L50$y, paste(round(L50$est, digits=2),"(Â±", round(1.96*L50$se, digits=2),")", sep=""), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
                        
                        text(x=L90$est, L90$y, expression(L[90]), pos=4, cex=1.2, font=3)                
                        text(x=L90$est, L90$y, paste(round(L90$est, digits=2)," (Â±", round(1.96*L90$se, digits=2),")", sep=""), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
                        
                        text(x=L10$est, L10$y, expression(L[10]), pos=4, cex=1.2, font=3)               
                        text(x=L10$est, L10$y, paste(round(L10$est, digits=2),"(Â±", round(1.96*L10$se, digits=2),")", sep=""), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
...
plot.logit=function(object, ylab="Probabilité",xlab="Longueur Ã  la fourche (mm)", se.pred=TRUE, 
        leg=TRUE, ref=TRUE, range.x=c("data","full"), warn.val=TRUE, main=NULL, enc.utf8=FALSE)
{

        if(enc.utf8){
                ylab=iconv(ylab,"utf-8")
                xlab=iconv(xlab,"utf-8")
        }
        if(enc.utf8 & !is.null(main)){
                main=iconv(main,"utf-8")
        }
        temp=model.frame(object)
        r.temp.x= range(temp[[2]])
        int.temp.x=r.temp.x[2]-r.temp.x[1]
        ...
        if(is.null(main)){
                if(enc.utf8){
                        title(iconv("Régression logistique","utf-8"), cex.main=1.5, line=2)
                        title(iconv("(± intervale de confiance Ã  95%)","utf-8"), cex.main=1, line=1)
                }else{
                        title("Régression logistique", cex.main=1.5, line=2)
                        title("(± intervale de confiance Ã  95%)", cex.main=1, line=1)
                }
        }
        if(se.pred){
                polygon(c(new.dat[,1],rev(new.dat[,1])), c(fit.se$upper,rev(fit.se$lower)), col=gray(0.9), border=NA)
        ...
        if(ref){
                ...
                if(enc.utf8){
                        ...
                        text(x=L50$est, L50$y, expression(L[50]), pos=4, cex=1.2, font=3)                
                        text(x=L50$est, L50$y, iconv(paste(round(L50$est, digits=2),"(±", round(1.96*L50$se, digits=2),")", sep=""),"utf-8"), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
                        
                        text(x=L90$est, L90$y, expression(L[90]), pos=4, cex=1.2, font=3)                
                        text(x=L90$est, L90$y, iconv(paste(round(L90$est, digits=2)," (±", round(1.96*L90$se, digits=2),")", sep=""),"utf-8"), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
                        
                        text(x=L10$est, L10$y, expression(L[10]), pos=4, cex=1.2, font=3)               
                        text(x=L10$est, L10$y, iconv(paste(round(L10$est, digits=2),"(±", round(1.96*L10$se, digits=2),")", sep=""),"utf-8"), 
                                srt=0, adj=c(1.2,-0.5),cex=0.8, font=2)
                }else{
                        text(x=L50$est, L50$y, expression(L[50]), pos=4, cex=1.2, font=3)                
