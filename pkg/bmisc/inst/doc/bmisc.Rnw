%\VignetteIndexEntry{bmisc's miscellaneous functions}
...
\item[Depends]\AsIs{car, lattice, zoo, gdata, robustbase, methods}
\item[License]\AsIs{LGPL >= 2.0}
\end{description}
\newpage
...
\inputencoding{utf8}
\newpage
\HeaderA{att.strp}{Attibutes stripper}{att.strp}
%
...
\begin{Arguments}
\begin{ldescription}
\item[\code{data}] the name of an object (\code{vector, matrix or data.frame})
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
This function strips a object of its attributes. In the case of a \code{vector}, all attributes are removed. 
For a \code{matrix}, only \code{c('dim','dimnames')} are kept. When \code{att.strp} is used on a \code{data.frame}, 
...
\inputencoding{utf8}
\newpage
\HeaderA{lib.code}{Retreives the code for 'lib()'.}{lib.code}
%
\begin{Description}\relax
...
\begin{verbatim}
## Default S3 method:
 norm.test(x, title=NULL, sk=c("G1","b1","mc"), type)
\end{verbatim}
\end{Usage}
%
\begin{Arguments}
\begin{ldescription}
\item[\code{x }] numeric vector or an object of class \code{"lm"} (i.e. lm, aov, glm, gam, ...).
\item[\code{title }] the title at the top of the results. Default is "Normality Tests".
\item[\code{sk }] type of skewness used in D'Agostino skewness test. Can be \code{"G1"},\code{"b1"} or \code{"mc"} (default). Read details.
...
Shapiro-Wilk will be far from correctly testing the \eqn{H{_0}}{} hypothesis.

Given samples from a population, the equation for the sample skewness \eqn{g{_1}}{}
is a biased estimator of the population skewness. The use of \eqn{G{_1}}{} or \eqn{b{_1}}{} is advisable.
For large samples, the various skewness estimates yield similar results. For small
normal distributed samples, \eqn{b{_1}}{} is less biased than \eqn{G{_1}}{}.
However, for small non-normal distributed samples, \eqn{G{_1}}{} is less biased than \eqn{b{_1}}{}.
...
Here, d'Agostino skewness test is based on \code{mc} by default:
...
where \eqn{m{_3}}{} is the sample third central moment, and \eqn{m{_2}}{} is the sample variance.

This is the typical definition used in many older textbooks.

\item[\eqn{G{_1}=}{}] 
\eqn{g{_1} * [k{_3}/(k{_2}^{(3/2)})] = g{_1} * [sqrt{n(n-1)} / (n-2)]}{}.

where \eqn{k{_3}}{} is the unique symmetric unbiased estimator of the third
cumulant and \eqn{k{_2}}{} is the symmetric unbiased estimator of the second cumulant.

Used in SAS and SPSS.

\item[\eqn{b{_1}=}{}] 
\eqn{m{_3} / s^3 = g_1 ((n-1)/n)^{(3/2)}}{}.
...
\item[\code{Zb\eqn{_1}{} }] D'Agostino Skewness results
\item[\code{Zb\eqn{_2}{} }] Anscombe-Glynn Kurtosis results
\item[\code{Chi\eqn{^2}{} }] D'Agostino Pearson results
...
%
\begin{SeeAlso}\relax
\code{residuals}, \code{residuals.lm}, \code{residuals.glm}, and \code{residuals.gam}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{SeeAlso}\relax
\code{pairwise.t.test}, \code{mc.long}, \code{DTK.test}, \code{TukeyHSD}, and \code{glht}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\begin{Usage}
\begin{verbatim}
YPR.l(fsel.type, last.age, vonB, l.start, LW, F.max, F.incr.YPR, 
      M, full.mat, f.MSP)
\end{verbatim}
\end{Usage}
%
...
\item[\code{vonB}] a vector containing \code{c(Linf,K)} from von Bertalanffy grotwh curve. 

\item[\code{l.start}] length at the starting age

\item[\code{LW}] a vector containing \code{c(alpha,beta)} from length-weight curve.

\item[\code{F.max}] maximum value of instantaneous rate of fishing mortality (\code{F})

\item[\code{F.incr.YPR}] increment value to be used for YPR calculation

\item[\code{M}] instantaneous rate of natural mortality (\code{M})

\item[\code{full.mat}] fully mature length value

\item[\code{f.MSP}] reference point defined as the fraction of maximum spawning potential
...
YPR.l(fsel.type=list("ramp",650,900), vonB=c(coeffs3[1],coeffs3[2]), 
      l.start=72.53,last.age=25, LW=c(coeffs2[2],coeffs2[1]),F.max=2, 
      F.incr.YPR=0.01, M=0.2, full.mat=900, f.MSP=0.4) 
